...
categories: Web
date: 2022-02-07 16:32:12 +08:00
...
Also, in practice we use a technique called "virtual nodes" in consistent hashing. A physical node might be mapped to multiple virtual nodes on the ring to achieve a fairer spread of keys among nodes. But in a large-scale system with thousands of nodes, if each node has to be mapped to hundreds or even thousands of virtual nodes, these metadata would take up a lot of memory.

To address these problems, Google proposed an alternative algorithm in 2014, called [*jump consistent hash*][jump]. The algorithm is very simple. The pseudo-code takes only 5 lines. It has much better key distribution and no memory overhead comparing to the original consistent hashing. And it's much faster! You can read the [paper][jump] to see the benchmark and how it achieve these results with a relatively simply algorithm.
