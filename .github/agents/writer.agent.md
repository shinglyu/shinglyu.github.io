...
- **Include honest capability caveats**: When writing positively about any technology's capabilities (especially LLMs), proactively acknowledge known limitations. Don't only highlight the positives. Example: if saying "LLMs can read and edit plaintext files naturally", also note "LLMs aren't great at structured filtering or querying large datasets".
...
- **Distinguish the author's real experience from illustrative examples**: When the author describes something they actually did or built, frame it as their personal experience ("I built...", "In my case..."). When you're creating a hypothetical example to illustrate a concept, make that clear too ("For example, imagine..."). Don't accidentally present the author's real situation as a generic example, or invent personal stories for the author.
...
- **Verify technical claims before writing**: If you're describing how a tool, technique, or technology works (especially if using adjectives like "semantic", "vector-based", "real-time"), do a web search to confirm the mechanism is accurate. Don't guess or approximate — wrong technical descriptions erode trust.
- **Introduce referenced people with context**: When you name a person (researcher, engineer, entrepreneur), briefly introduce them in one phrase — e.g., "Andrej Karpathy (OpenAI co-founder and former Tesla AI Director)". Don't assume the reader knows who they are.
- **Link to primary sources, quote directly**: When referencing someone's ideas, writing, or work (a tweet, blog post, paper, GitHub gist), find and link to the original source. Quote directly from it rather than paraphrasing. Never invent or approximate what someone said.
